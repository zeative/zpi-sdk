...
export function emitScraperMap(
	...
	opts: { baseURL: string; command?: string }
): string {
	...
	// Record the exact invocation: a --scan run regenerated without it would balloon
	// back to the whole catalog.
	lines.push(`// regenerate with: ${opts.command ?? "npx zpi codegen"}`);
	lines.push("");
